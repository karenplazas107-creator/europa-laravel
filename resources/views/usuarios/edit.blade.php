...
@section('title', 'Editar Usuario – Almacén Europa')
...
<div class="usr-form-page">
    ...
    <div class="usr-form-card">
        <div class="usr-form-card__header">
            ...
            <div>
                <h2 class="usr-form-card__title">Editar Usuario: {{ $usuario->nombre }} {{ $usuario->apellido }}</h2>
                <p class="usr-form-card__sub">Modifica los datos personales, cambia el rol o restablece la contraseña.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('usuarios.update', $usuario->usuario) }}" class="usr-form">
            @csrf
            @method('PUT')
            ...
            <div class="usr-form-section">
                ...
                @php
                    $rawRol = $usuario->rol_normalizado;
                    if (in_array($rawRol, ['admin', 'administrador'])) {
                        $actualRol = 'administrador';
                    } elseif (in_array($rawRol, ['auxiliar_bodega', 'bodega', 'auxiliar de bodega'])) {
                        $actualRol = 'auxiliar_bodega';
                    } else {
                        $actualRol = $rawRol;
                    }
                    $selectedRol = old('rol', $actualRol);
                @endphp
            ...
            <div class="usr-form-section">
                ...
                <p class="usr-section-desc">Deja estos campos vacíos si deseas conservar la contraseña actual del usuario.</p>

                <div class="usr-grid-2">
                    <div class="usr-form-group">
                        <label class="usr-label" for="password">Nueva Contraseña</label>
                        <input
                            type="password"
                            name="password"
                            id="password"
                    ...
                    <div class="usr-form-group">
                        <label class="usr-label" for="password_confirmation">Confirmar Nueva Contraseña</label>
                        <input
                            type="password"
